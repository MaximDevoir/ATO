...
async function main() {
  ...
  function assertUProjectInCwd(): void {
    ...
    if (uprojects.length > 1) {
      console.warn(
        `[BuildGraph] Multiple .uproject files found in ${cwd}, using ${uprojects[0]}`
      );
    }
  }
  ...
  function findUProject(startDir: string): string | null {
    ...
    while (true) {
      ...
      if (uprojects.length > 0) {
        if (uprojects.length > 1) {
          console.warn(
            `[BuildGraph] Multiple .uproject files found in ${current}, using ${uprojects[0]}`
          );
        }
  ...
  if (process.env.PROJECT_FILE) {
    ...
    if (!found) {
      throw new Error(
        "[BuildGraph] Could not find .uproject. " +
        "Set PROJECT_FILE or run from within a project directory."
      );
    }
  ...
  function findShell(): { exe: string; args: string } {
    if (process.platform === 'win32') {
      const candidates = ['C:/Program Files/PowerShell/7/pwsh.exe', 'C:/Windows/System32/WindowsPowerShell/v1.0/powershell.exe', 'pwsh', 'powershell'];
      ...
      throw new Error('[BuildGraph] Could not find a shell (PowerShell). Install PowerShell (pwsh) or ensure a compatible shell is on PATH.');
    }
  ...
  function findBuildGraphSchema(engineDir: string): string | null {
    ...
    if (override) {
      ...
      if (!exists(normalized)) {
        console.warn(
          `[BuildGraph] SCHEMA_PATH is set but file does not exist: ${normalized}`
        );
      } else {
    ...
    console.log(
      `[BuildGraph] Schema.xsd not found (override or EngineDir), using base namespace only`
    );
...
// Output always goes to project (cwd)
  ...
  if (fs.existsSync(outputPath) && process.env.FORCE_OVERWRITE !== '1') {
    if (!process.stdin.isTTY) {
      console.log('[BuildGraph] Output already exists and running in non-interactive mode. Set FORCE_OVERWRITE=1 to overwrite automatically.');
      return;
    }
    const rl = readline.createInterface({input: process.stdin, output: process.stdout});
    const answer: string = await new Promise((resolve) => {
      rl.question(`[BuildGraph] Output already exists at ${outputPath}. Overwrite? (y/N): `, (ans) => resolve(ans ?? ''));
    });
    rl.close();
