...
describe('BuildCsInjector', () => {
  it('injects includes, wrapper, and constructor call idempotently', () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'uapm-buildcs-'));
    ...
    expect(source.match(/UAPM-BEGIN AwesomeInventory:module-includes/g)?.length ?? 0).toBe(1);
    expect(source.match(/UAPM-BEGIN AwesomeInventory:module-class-body/g)?.length ?? 0).toBe(1);
    expect(source.match(/UAPM-BEGIN AwesomeInventory:module-constructor/g)?.length ?? 0).toBe(1);
    expect(source).toContain('UAPM_');
...
describe('TargetCsInjector', () => {
  it('fails when TargetType assignment is missing', () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'uapm-targetcs-'));
...
describe('UProjectInjector', () => {
  it('adds and enables plugins while preserving existing entries', () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'uapm-uproject-'));
...
describe('PostinstallModuleSelector', () => {
  it('discovers modules from uproject when explicit postinstall modules are not configured', () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'uapm-module-selector-'));
    fs.writeFileSync(
