...
import type { LockedPackage, UAPMLockfile } from './UAPMLockfile';

export interface LockfileRepository {
  getPath(cwd: string): string;
  exists(cwd: string): boolean;
  read(cwd: string): UAPMLockfile;
  write(cwd: string, lockfile: UAPMLockfile): void;
}

export class TOMLLockfileRepository implements LockfileRepository {
  getPath(cwd: string) {
    return path.join(cwd, 'uapm.lock');
  }
  ...
  read(cwd: string): UAPMLockfile {
    ...
    try {
      ...
      if (error && typeof error === 'object' && 'line' in error) {
        ...
        throw new Error(`[uapm] Failed to parse ${lockfilePath}:\n${pretty}`);
      }
      throw new Error(
        `[uapm] Failed to parse ${lockfilePath}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }

  write(cwd: string, lockfile: UAPMLockfile) {
...
function normalizeLockedPackage(value: unknown, lockfilePath: string): LockedPackage {
  if (!value || typeof value !== 'object') {
    throw new Error(`[uapm] Invalid package entry in ${lockfilePath}`);
  }

  const entry = value as Partial<LockedPackage>;
  if (!entry.name || !entry.source || !entry.version || !entry.hash) {
    throw new Error(`[uapm] Lockfile package entry missing required keys in ${lockfilePath}`);
  }
