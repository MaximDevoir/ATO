  ...
  private resolveModules(projectRoot: string, expectedModules: string[]) {
    ...
    for (const filePath of files) {
      ...
      if (discovered.has(parsed.classInfo.className)) {
        throw new Error(
          `[uapm] Duplicate ModuleRules class '${parsed.classInfo.className}' in ${discovered.get(parsed.classInfo.className)} and ${filePath}`,
        );
      }
    ...
    if (missing.length > 0) {
      throw new Error(`[uapm] Missing module Build.cs files for: ${missing.join(', ')}`);
    }
