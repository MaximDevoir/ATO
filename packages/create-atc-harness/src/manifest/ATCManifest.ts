...
export function parseAndValidateATCManifest(rawManifest: string, sourcePath: string) {
  ...
  try {
    ...
    throw new Error(
      `[create-atc-harness] Failed to parse uapkg.json at ${sourcePath}: ${error instanceof Error ? error.message : String(error)}`,
    );
  }

  if (!parsed || typeof parsed !== 'object') {
    throw new Error(`[create-atc-harness] uapkg.json at ${sourcePath} must be a JSON object`);
  }

  const manifest = parsed as Partial<ATCManifest>;
  if (manifest.type !== 'plugin') {
    throw new Error(`[create-atc-harness] uapkg.json at ${sourcePath} must contain type: "plugin"`);
  }

  if (typeof manifest.harness !== 'string' || !manifest.harness.trim()) {
    throw new Error(`[create-atc-harness] uapkg.json at ${sourcePath} must contain a non-empty string harness value`);
  }
